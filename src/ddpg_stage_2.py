...
def load(local_name, model_episode):
    actor.load_state_dict(
        torch.load(
            dirPath + '/save_model/stage_02/'+local_name+'/checkpoint_actor_'+str(model_episode)+ '.pkl',
    ...
    critic.load_state_dict(
        torch.load(
            dirPath + '/save_model/stage_02/'+local_name+'/checkpoint_critic_'+str(model_episode)+ '.pkl',
            map_location=torch.device(device)
        )
    )
...
def subplot(R, P, A, Q):
    ...
    plt.savefig(dirPath + '/save_plot/stage_02/'+timestr+'_plot.png')
    plt.close()
    #plt.show()
...
def save_data(E, R, T, C, G, A):
    ...
    file = open(dirPath + '/save_file/stage_02/training/stage02_training.csv', 'a+')
...
def save_test(T, E, C, G, S):
    ...
    file = open(dirPath + '/save_file/stage_02/test_ddpg_stage02b.csv', 'a+')
    writer = csv.writer(file)
...
if __name__ == '__main__':
    ...
    rospy.init_node('deepbot_stage_02', anonymous=True, log_level=rospy.INFO, disable_signals=False)
    ...
    if training:
        ...
        for episode in range(load_episode + 1, MAX_EPISODES):
            ...
            if ep_reward > best_reward:
                torch.save(actor.state_dict(), dirPath + '/save_model/stage_02/checkpoint_actor_'+str(episode)+'.pkl') 
                torch.save(critic.state_dict(), dirPath + '/save_model/stage_02/checkpoint_critic_'+str(episode)+'.pkl')
