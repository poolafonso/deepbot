...
from env.environment_stage_4 import Env
...
action_space_high = 1.5
action_space_low = -1.5
...
def subplot(R, P, A, Q):
    ...
    plt.savefig(dirPath + '/save_plot/stage_04/'+timestr+'_plot.png')
    plt.close()
    #plt.show()
...
def save_data(E, R, T, C, G, A):
    ...
    file = open(dirPath + '/save_file/stage_04/corridor_stage04.csv', 'a+')
...
def save_test(T, E, C, G, S):
    ...
    file = open(dirPath + '/save_file/stage_04/test_ddpg_stage04b.csv', 'a+')
    writer = csv.writer(file)
...
if __name__ == '__main__':
    ...
    rospy.init_node('deepbot_stage_4', anonymous=True, log_level=rospy.INFO, disable_signals=False)
    ...
    for episode in range(load_episode + 1, MAX_EPISODES):
        ...
        if ep_reward > best_reward:
            torch.save(actor.state_dict(), dirPath + '/save_model/stage_04/checkpoint_actor_'+str(episode)+'.pkl') 
            torch.save(critic.state_dict(), dirPath + '/save_model/stage_04/checkpoint_critic_'+str(episode)+'.pkl')
        ...
        if (episode % PRINT_EVERY) == (PRINT_EVERY-1):    # print every print_every episodes
            print('[%6d episodes, %8d total steps] average reward for past {} iterations: %.3f'.format(PRINT_EVERY) %
