...
from env.environment_stage_1 import Env
...
def subplot(R, P, A, Q):
    ...
    plt.savefig(dirPath + '/save_plot/stage_01/'+timestr+'_plot.png')
    plt.close()
    #plt.show()
...
def save_data(E, R, T, C, G, A):
    ...
    file = open(dirPath + '/save_file/stage_01/stage01.csv', 'a+')
...
if __name__ == '__main__':
    ...
    rospy.init_node('deepbot_stage_1', anonymous=True, log_level=rospy.INFO, disable_signals=False)
    ...
    for episode in range(load_episode + 1, MAX_EPISODES):
        ...
        if ep_reward > best_reward:
            torch.save(actor.state_dict(), dirPath + '/save_model/stage_01/checkpoint_actor_'+str(episode)+'.pkl')
            torch.save(critic.state_dict(), dirPath + '/save_model/stage_01/checkpoint_critic_'+str(episode)+'.pkl')
